...
bool PathTree::addPath(std::string path, uint32_t unique_id, std::vector<uint32_t>& covered_ids){
    ...
    if (root == nullptr) {
        root = std::make_shared<Node>(dirs[0]);
        auto tmp = root;
        for (size_t i = 1;i < dirs.size();i++) {
            auto dir = dirs[i];
            auto next = std::make_shared<Node>(dir);
            tmp->addChild(next);
            tmp = next;
        }
    ...
    while (true) {
        ...
        if (found) {
            ...
            auto start = std::make_shared<Node>(dirs[i]);
            auto tmp = start;
            i++;
            for (;i < dirs.size();i++) {
                auto next = std::make_shared<Node>(dirs[i]);
                tmp->addChild(next);
                tmp = next;
            }
...
void PathTree::display(QTreeView* treeView){
    ...
    // model->clear();
    std::function<void(std::shared_ptr<Node>, QStandardItem*)> build = [&](std::shared_ptr<Node> node, QStandardItem* node_item){
        for (auto& child : node->children_) {
